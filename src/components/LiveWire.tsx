...
export function LiveWire({ title, eyebrow = 'Live wire', hint, keywords, limit = 6 }: Props) {
  ...
  return (
    <section>
      ...
      />
      {state.kind === 'live' || state.kind === 'delayed' ? (
        items.length === 0 ? (
          <EmptyState label="No matching headlines under this filter." />
        ) : (
        ...
        <EmptyState
          label={
            state.kind === 'mock'
              ? 'Live news wire not enabled in this build (VITE_DATA_MODE).'
              : 'Live news feed unavailable — moneycontrol.json failed to load.'
          }
        />
      )}
    </section>
...
function EmptyState({ label }: { label: string }) {
  return (
    <div className="px-4 py-8 rounded-2xl border border-dashed border-bordersoft bg-cream/60 text-center text-[12px] text-charcoal-mute">
      {label}
    </div>
  );
}
