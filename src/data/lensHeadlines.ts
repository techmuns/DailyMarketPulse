...
export const lensHeadlines: LensHeadline[] = [
  ...
  // ---------------- CUSTOM ----------------
  {
    id: 'c-1',
    lensType: 'custom',
    category: 'Custom · AI Pick',
    headline: 'Asian Paints filing + INR weakness create non-obvious read',
    shortContext: 'Selective hike interacts with FX move — net positive on margin.',
    fullContext:
      'A custom cross-signal: the 0.6% selective hike combined with INR drift gives the company a roughly 110 bps margin buffer at current crude levels — non-obvious unless the two are combined.',
    whyItMatters:
      'Worth a thesis-level revision rather than a tactical move. Models likely undershooting Q4 EBITDA.',
    signal: 'support',
    affectedCompanies: ['ASIANP'],
    affectedSectors: ['Paints'],
    sourceType: 'Reliable media',
    action: 'PM bench review',
    timestamp: '08:25 IST',
  },
  {
    id: 'c-2',
    lensType: 'custom',
    category: 'Custom · Saved view',
    headline: 'Rate-sensitive cluster: 5-day breadth turning',
    shortContext: 'Banks + NBFC + auto financiers all green on a 5-day basis.',
    fullContext:
      'Saved breadth view: rate-sensitive cluster shows 14 of 20 names green on a rolling 5-day basis vs 6 of 20 a fortnight ago. CPI tailwind is the simplest explanation.',
    whyItMatters:
      'Cluster-level confirmation often precedes a leadership rotation. Position-sizing trigger active.',
    signal: 'support',
    affectedCompanies: ['HDFCB', 'BAJFIN', 'CHOLAFIN'],
    affectedSectors: ['Banks', 'NBFC', 'Auto finance'],
    sourceType: 'Reliable media',
    action: 'Open breadth dashboard',
    timestamp: '08:10 IST',
  },
  {
    id: 'c-3',
    lensType: 'custom',
    category: 'Custom · Concall watch',
    headline: 'Crude pass-through commentary — six concalls this week',
    shortContext: 'Paints, FMCG, aviation, chemicals all reporting in 5 sessions.',
    fullContext:
      'Concentration of pricing-power commentary in a single week makes for an efficient input-cost check across the consumption stack.',
    whyItMatters:
      'Single thematic listen-through; useful for confirming or rejecting the FY input-cost view.',
    signal: 'monitor',
    affectedCompanies: ['ASIANP', 'HINDUNILVR', 'INDIGO'],
    affectedSectors: ['Paints', 'FMCG', 'Aviation', 'Chemicals'],
    sourceType: 'Company source',
    action: 'Schedule concall block',
    timestamp: '07:48 IST',
  },
  {
    id: 'c-4',
    lensType: 'custom',
    category: 'Custom · Risk lens',
    headline: 'EM outflows two weeks running — watch INR + bank index',
    shortContext: 'India equity ETFs continue to see redemptions; thin so far.',
    fullContext:
      'India-dedicated ETF flows have been -$420m over the last two weeks. Net thin in context of $35bn AUM but a directional change worth tracking.',
    whyItMatters:
      'Flow signal more useful in conjunction with yield and INR moves than in isolation. Becomes meaningful if it accelerates.',
    signal: 'risk',
    affectedCompanies: ['HDFCB', 'ICICIBANK', 'KOTAKBANK'],
    affectedSectors: ['Banks'],
    sourceType: 'Reliable media',
    action: 'Watch + 5d threshold alert',
    timestamp: '07:25 IST',
  },
];
